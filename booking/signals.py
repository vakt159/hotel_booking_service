...
def booking_payment_signal(sender, instance: Booking, created, **kwargs):
    ...
    if created and instance.status == Booking.BookingStatus.BOOKED:
        transaction.on_commit(
            lambda: create_stripe_payment_task.delay(instance.id, Payment.PaymentType.BOOKING)
        )
        logger.info(f"Created BOOKING payment task for Booking {instance.id}")
        return

    if instance.status == Booking.BookingStatus.COMPLETED:
        has_booking_payment = instance.payments.filter(type=Payment.PaymentType.BOOKING).exists()
        ...
        if instance.actual_check_out_date and instance.actual_check_out_date > instance.check_out_date:
            has_overstay = instance.payments.filter(type=Payment.PaymentType.OVERSTAY_FEE).exists()
            if not has_overstay:
                transaction.on_commit(
                    lambda: create_stripe_payment_task.delay(instance.id, Payment.PaymentType.OVERSTAY_FEE)
                )
                logger.info(f"Created OVERSTAY_FEE payment task for Booking {instance.id}")

    elif instance.status == Booking.BookingStatus.NO_SHOW:
        has_no_show = instance.payments.filter(type=Payment.PaymentType.NO_SHOW_FEE).exists()
        if not has_no_show:
            transaction.on_commit(
                lambda: create_stripe_payment_task.delay(instance.id, Payment.PaymentType.NO_SHOW_FEE)
            )
        ...
        has_cancel_fee = instance.payments.filter(type=Payment.PaymentType.CANCELLATION_FEE).exists()
        hours_to_checkin = (instance.check_in_date - timezone.now().date()).total_seconds() / 3600
        if not has_cancel_fee and hours_to_checkin < 24:
            transaction.on_commit(
                lambda: create_stripe_payment_task.delay(instance.id, Payment.PaymentType.CANCELLATION_FEE)
            )
            logger.info(f"Created CANCELLATION_FEE payment task for Booking {instance.id}")
